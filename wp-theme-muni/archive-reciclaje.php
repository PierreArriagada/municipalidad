...
/**
 * Plantilla para el listado de Reciclaje (Archivo)
 *
    ...
    <div class="container">
        <header class="page-header" style="margin-bottom: 3rem; text-align: center;">
            ...
            <div class="archive-description" style="color: var(--color-text-light, #64748b); font-size: 1.1rem;">
                Infórmate sobre nuestras campañas medioambientales y puntos limpios.
            </div>
        </header>
            ...
            <div class="tripticos-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 2rem;">
                <?php while ( have_posts() ) : the_post(); ?>
                    <article id="post-<?php the_ID(); ?>" <?php post_class( 'reciclaje-card' ); ?> style="background: var(--color-surface, #fff); border-radius: var(--radius-lg, 12px); overflow: hidden; box-shadow: var(--shadow-md, 0 4px 6px rgba(0,0,0,0.1)); transition: transform 0.3s ease;">
                        <?php 
                        $thumb_url = muni_get_post_image( get_the_ID(), 'reciclaje.webp' );
                        ?>
                        ...
                        <div class="reciclaje-info" style="padding: 1.5rem; text-align: center;">
                            <h3 class="reciclaje-title" style="margin: 0 0 1rem 0; font-size: 1.25rem;">
                                <a href="<?php the_permalink(); ?>" style="color: var(--color-text, #1e293b); text-decoration: none;"><?php the_title(); ?></a>
                            </h3>
                            <a href="<?php the_permalink(); ?>" class="reciclaje-link" style="display: inline-block; padding: 0.5rem 1.2rem; background: var(--color-primary, #003399); color: #fff; border-radius: 50px; text-decoration: none; font-size: 0.9rem; font-weight: 500;">
                                Ver información ➔
                            </a>
                        </div>
                    </article>
        ...
        <?php else : ?>
            <div class="no-results" style="text-align: center; padding: 4rem 0;">
                <p style="font-size: 1.2rem; color: var(--color-text-light, #64748b);"><?php esc_html_e( 'Pronto subiremos nuevas campañas de reciclaje.', 'muni-santa-juana' ); ?></p>
            </div>
        <?php endif; ?>
    </div>
</main>
