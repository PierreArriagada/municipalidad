-- ============================================================================
-- SCRIPT DE LIMPIEZA Y MIGRACIÓN: juana25_wp.sql → prueba.santajuana.cl
-- ============================================================================
-- INSTRUCCIONES:
--   1. En phpMyAdmin de prueba.santajuana.cl, BORRA todas las tablas wp_* existentes
--   2. Importa juana25_wp.sql (pestaña Importar)
--   3. Luego ejecuta ESTE script (pestaña SQL → pegar todo → Continuar)
-- ============================================================================
-- Dominio antiguo: new.santajuana.cl (prefijo BpyNWTw_)
-- Dominio nuevo:   prueba.santajuana.cl (prefijo wp_)
-- ============================================================================
...
-- ============================================================================
-- PASO 0: ELIMINAR TABLAS wp_* PREEXISTENTES DEL HOSTING
-- ============================================================================
-- Si el hosting ya tenía WordPress instalado, estas tablas pueden existir
-- y bloquear el RENAME del paso 1.
...
-- Elementor submissions (6 tablas)
...
-- Live Weather Station (15 tablas)
...
-- Custom Facebook Feed (4 tablas)
DROP TABLE IF EXISTS `BpyNWTw_cff_facebook_feed_locator`;
DROP TABLE IF EXISTS `BpyNWTw_cff_feed_caches`;
DROP TABLE IF EXISTS `BpyNWTw_cff_feeds`;
DROP TABLE IF EXISTS `BpyNWTw_cff_sources`;

-- ============================================================================
-- PASO 3: LIMPIAR POSTS — CONSERVAR SOLO NOTICIAS, ADJUNTOS Y PÁGINAS
-- ============================================================================

-- Eliminar todas las revisiones (~625 registros)
DELETE FROM `wp_posts` WHERE `post_type` = 'revision';

-- Eliminar tipos de post de plugins/tema viejo
...
-- ============================================================================
-- PASO 4: ACTUALIZAR URLs (new.santajuana.cl → prueba.santajuana.cl)
-- ============================================================================

-- Contenido de posts
UPDATE `wp_posts` SET `post_content` = REPLACE(`post_content`, 'https://new.santajuana.cl', 'https://prueba.santajuana.cl');
UPDATE `wp_posts` SET `post_content` = REPLACE(`post_content`, 'http://new.santajuana.cl', 'https://prueba.santajuana.cl');

-- GUIDs de posts
UPDATE `wp_posts` SET `guid` = REPLACE(`guid`, 'https://new.santajuana.cl', 'https://prueba.santajuana.cl');
UPDATE `wp_posts` SET `guid` = REPLACE(`guid`, 'http://new.santajuana.cl', 'https://prueba.santajuana.cl');

-- Excerpts
UPDATE `wp_posts` SET `post_excerpt` = REPLACE(`post_excerpt`, 'https://new.santajuana.cl', 'https://prueba.santajuana.cl');

-- Post meta (imágenes destacadas, URLs en campos personalizados)
UPDATE `wp_postmeta` SET `meta_value` = REPLACE(`meta_value`, 'https://new.santajuana.cl', 'https://prueba.santajuana.cl');
UPDATE `wp_postmeta` SET `meta_value` = REPLACE(`meta_value`, 'http://new.santajuana.cl', 'https://prueba.santajuana.cl');

-- Versiones JSON-escaped (Gutenberg blocks, Elementor data)
UPDATE `wp_posts` SET `post_content` = REPLACE(`post_content`, 'https:\\/\\/new.santajuana.cl', 'https:\\/\\/prueba.santajuana.cl');
UPDATE `wp_postmeta` SET `meta_value` = REPLACE(`meta_value`, 'https:\\/\\/new.santajuana.cl', 'https:\\/\\/prueba.santajuana.cl');

-- Opciones (serialized data)
UPDATE `wp_options` SET `option_value` = REPLACE(`option_value`, 'https://new.santajuana.cl', 'https://prueba.santajuana.cl')
WHERE `option_name` NOT IN ('siteurl', 'home');
UPDATE `wp_options` SET `option_value` = REPLACE(`option_value`, 'https:\\/\\/new.santajuana.cl', 'https:\\/\\/prueba.santajuana.cl')
WHERE `option_name` NOT IN ('siteurl', 'home');
UPDATE `wp_options` SET `option_value` = REPLACE(`option_value`, 'http://new.santajuana.cl', 'https://prueba.santajuana.cl')
WHERE `option_name` NOT IN ('siteurl', 'home');

-- User meta
UPDATE `wp_usermeta` SET `meta_value` = REPLACE(`meta_value`, 'https://new.santajuana.cl', 'https://prueba.santajuana.cl');

-- ============================================================================
-- PASO 5: CONFIGURAR OPCIONES DEL SITIO
-- ============================================================================

-- URLs del sitio (CRÍTICO)
UPDATE `wp_options` SET `option_value` = 'https://prueba.santajuana.cl' WHERE `option_name` = 'siteurl';
UPDATE `wp_options` SET `option_value` = 'https://prueba.santajuana.cl' WHERE `option_name` = 'home';
...
-- Eliminar opciones de plugins viejos
...
DELETE FROM `wp_options` WHERE `option_name` LIKE 'cff_%';
...
-- Eliminar relaciones de posts que ya no existen
DELETE tr FROM `wp_term_relationships` tr
LEFT JOIN `wp_posts` p ON tr.object_id = p.ID
WHERE p.ID IS NULL;

-- Eliminar terms de Elementor/tema viejo (conservar: 1=Noticias, 4=Concursos, 5=Eventos, 6=Historia)
...
-- ============================================================================
-- PASO 7: LIMPIAR METADATA DE USUARIOS
-- ============================================================================
...
-- Corregir prefijo de capabilities (BpyNWTw_ → wp_)
...
-- ============================================================================
-- PASO 8: LIMPIAR POSTMETA DE PLUGINS
-- ============================================================================
...
-- ============================================================================
-- PASO 9: ASIGNAR NOTICIAS HUÉRFANAS A CATEGORÍA "NOTICIAS"
-- ============================================================================
-- Posts que no tienen ninguna categoría asignada se les asigna "Noticias" (term_taxonomy_id=1)
...
-- Recalcular contador final de Noticias
...
-- ============================================================================
-- 1. Sube la carpeta wp-theme-muni/ a wp-content/themes/wp-theme-muni/ via FTP
-- 2. Sube la carpeta uploads/ a wp-content/uploads/ via FTP
-- 3. Ve a https://prueba.santajuana.cl/wp-admin/
-- 4. Ve a Ajustes > Enlaces Permanentes > clic en "Guardar cambios"
-- 5. Ejecuta verificar_migracion_prueba.sql para confirmar que todo está OK
-- ============================================================================
